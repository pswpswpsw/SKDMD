...
from lib_analytic_model import F_omib_2d_system
...
def main(case):
    ...
    if case == '2d_simple_lusch2017':
    ...
    elif case == '2d_lezhengthu':

        noise_level = 0
        F_governing = F_omib_2d_system
        phase_space_range = [[-0.2, 0], [-.5, 5]]
        num_samples_each_dim = 10
        range_of_X = np.array(phase_space_range)

        # generate data using ClassGenerateXXDotFromPhysics
        data_generator = ClassGenerateXXDotFromPhysics(directory='../EXAMPLES/lezhengthu/', case_name=case,
                                                       noise_level=noise_level)
        data_generator.make_case_dir()
        data_generator.samplingX_Xdot(F=F_governing, range_of_X=range_of_X, num_samples_each_dim=num_samples_each_dim)
        data_generator.save_X_Xdot()
        ...
        ## test visualization
        data_generator._test_visualize_train_with_odeint(F=F_governing)
