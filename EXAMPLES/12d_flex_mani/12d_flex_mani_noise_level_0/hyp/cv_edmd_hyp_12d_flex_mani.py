...
def main(case, noise_level):
    ...
    # select EDMD hyperparameter for sweeping
    kernel = 'gaussian'
    ...
    if rank == 0:
        ...
        # num_feature_list = np.arange(100, 1600, 200)

        ## practice
        sigma_list = np.logspace(0, 5, 40)
        num_feature_list = np.arange(300, 400, 30)


        # check to make sure that EDMD is not naively overfitted
        assert np.max(num_feature_list) <= total_numer_data

        # prepare mpi job distribution
        mpi_job_total_list = []
        for sigma in sigma_list:
            for nf in num_feature_list:
                mpi_job_total_list.append({'sigma':sigma, 'nf':nf})
    ...
    for arg_dict in mpi_job_each_rank:
        ...
        print('================================================')
        print('for sigma = ', sigma, ' number of RFF features = ', nf)
        ...
        mpi_result_each_rank.append({'sigma': sigma,
                                     'rank':nf, ## NOTE: HERE WE SIMPLY TAKE RANK AS NF...
